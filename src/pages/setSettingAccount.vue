...
<section class="otcHome">
  <div class="accountSettingWrapper">
    <div class="accountSettingWrapper_left">
      <div style="height:77px;width:243px;border-bottom:1px solid #e4e4e9;">
        <p style="color:#372632;font-size:18px;font-weight:bold;">{{merchantName}}</p>
        <p style="color:#979595;font-size:14px;margin-top:10px;">UID : {{accountUserId}}</p>
      </div>
      <div style="margin-top:45px;height:77px;width:243px;border-bottom:1px solid #e4e4e9;">
        <p style="color:#979595;font-size:14px;">总成单：
          <span style="color:#000;">{{numberTime}}次</span>
        </p>
      </div>
      <div style="margin-top:45px;height:77px;width:243px;">
        <p style="color:#979595;font-size:14px;">
          本账号于{{createTime | timeFilter}} 注册
        </p>
      </div>
    </div>
    <div class="accountSettingWrapper_right">
      <div class="top">
        <div style="#353535;font-size:16px;font-weight:bold;padding-top:37px;padding-bottom:20px;border-bottom:1px solid #f5f5f5;">账户与安全</div>
        <div style="height:55px;line-height:55px;border-bottom:1px solid #f5f5f5;">
          <img style="float:left;margin-top:20px;margin-right:10px;vertical-align: middle;" src="../assets/account/email.png" alt="icon">
          <span class="commleft">邮箱</span>
          <span class="commrightApprove" @click="goBindEmail">{{emailStatus | approveParse}}</span>
          <i v-if="emailStatus" style="float:right;margin-top:20px;margin-right:10px;color:#06bf8a;" class="el-icon-success"></i>
          <i v-if="!emailStatus" style="float:right;margin-top:20px;margin-right:10px;color:red;" class="el-icon-error"></i>
        </div>
        <div style="height:55px;line-height:55px;border-bottom:1px solid #f5f5f5;">
          <img style="float:left;margin-top:20px;margin-right:10px;vertical-align: middle;" src="../assets/account/shiming.png" alt="icon">
          <span class="commleft">实名认证</span>
          <span class="commrightApprove">已认证</span>
        </div>
        <div style="height:55px;line-height:55px;border-bottom:1px solid #f5f5f5;">
          <img style="float:left;margin-top:20px;margin-right:10px;vertical-align: middle;" src="../assets/account/login.png" alt="icon">
          <span class="commleft">登录密码</span>
          <span @click="changePass" class="commright">修改</span>
        </div>
        <div style="height:55px;line-height:55px;">
          <img style="float:left;margin-top:20px;margin-right:10px;vertical-align: middle;" src="../assets/account/login.png" alt="icon">
          <span class="commleft">资金密码</span>
          <span @click="changePass" class="commright">修改</span>
        </div>
      </div>
      <div class="bom">
        <p style="color:#353535;font-size:16px;font-weight:bold;">支付方式</p>
        <div style="height:50px;padding-top:12px;border-bottom:1px solid #f5f5f5;">
          <span style="color:#bab9b9;font-size:14px;float:left;">请务必使用您本人的实名账户，被开启的支付方式将在交易时向对方展示</span>
          <span class="addbank" @click="addPayMoney" style="color:#478ce9;font-size:16px;float:right;cursor: pointer;">添加</span>
        </div>
        <div style="height:63px;line-height:63px;border-bottom:1px solid #f5f5f5;"  v-if="bankList.accountNo">
          ...
          <span @click="changeBankInfo" class="addbank" style="color:#478ce9;font-size:16px;float:right;margin-right:40px; cursor: pointer;">修改</span>
          <span :title="bankList.accountNo + ' ' + bankList.name + ' ' + bankList.bankName + ' ' + bankList.branchName" style="text-align:right;color:#000;overflow:hidden;width:500px;font-size:14px;float:right;margin-right:40px;text-overflow: ellipsis;white-space: nowrap;">{{bankList.accountNo}}    {{bankList.name}}    {{bankList.bankName}}    {{bankList.branchName}}</span>
        </div>
        <div style="height:63px;line-height:63px;" v-if="zhifuList.accountNo">
          ...
          <span @click="changezhifuInfo" class="addbank" style="color:#478ce9;font-size:16px;margin-right:40px;float:right; cursor: pointer;">修改</span>
          <span :title="zhifuList.accountNo + ' ' + zhifuList.name " style="text-align:right;color:#000;overflow:hidden;width:500px;font-size:14px;float:right;margin-right:40px;text-overflow: ellipsis;white-space: nowrap;">{{zhifuList.accountNo}}    {{zhifuList.name}}   </span>
        </div>
      </div>
    </div>
      <!-- 添加支付方式弹窗 -->
...
export default {
  data () {
    return {
      ...
      bankList:{}, // 保存后台获取银行卡信息
      zhifuList:{}, // 保存后台获取支付宝信息
      bankName: '银行卡号',
...
<style lang='scss' scoped>
  .accountSettingWrapper{
    ...
    width: 1200px;
    margin: 0 auto;
    .accountSettingWrapper_left{

      float: left;
      width: 368px;
      height: 300px;
    }
    .accountSettingWrapper_right{
      float: left;
